...
> In modern OS, all the things should be in control state, so even some program tried to access the addresses out-of-bound, then need to throw exceptions, but the operation of "throw exceptions" also be controlled in the OS so can still be treated as in charged.

1. OS need to have *privileged mode* and *user mode*, first one can execute system call and the one after cannot.
2. Hardware need to provide *base and bounds registers* themselves to make management of addresses which one process can touch, also need to provide ability for OS to change these registers when program changed.
3. CPU must be able to throw exceptions and arrange exceptions of OS.

### 15.5 Operating System Issues

OS need to provides those functions:

1. When a process is created, the OS need to take action, like finding space for the program
2. OS must aware of some work of process terminated, like when process itself quit or be terminated by OS,because OS needs to clean the space of killed process.
3. When context switch occurs, the OS must performa a few additional steps for the process because there is only one pair of *base and bounds registers* on per CPU, so each time switch context then need to clean and reload values.
4. OS must provide exception handlers for process, to raise exceptions when needed like touch the address which exceed the range the program can touch.

![image-20210908094444209](../img/2021-07-22-操作系统导论/image-20210908094444209.png)

# 16. Segmentation

if we just put the whole memory of one program into the physical memory, then it would be wasteful because maybe one program only use some memory now but requires a large amount of memory.
