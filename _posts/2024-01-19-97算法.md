...
![](../img/assets_2024-01-19-97算法/2024-02-02-18-29-51-image.png)

后序就是从未知点出发，推导到已知点，主要是用DFS来做。

![](../img/assets_2024-01-19-97算法/2024-02-02-18-32-55-image.png)

### 结构顺序

分为邻接顺序和访问顺序。

邻接顺序，就是**一个节点将数据传输给其邻接节点。** 数据传输在原有的数据结构上面进行，不进行数据变换。

![](../img/assets_2024-01-19-97算法/2024-02-02-18-34-33-image.png)

访问顺序，就是将原始的数据结构转换成线性序列，在线性序列上面进行传输，一般用于将非线性数据结构转换成线性结构。

通常设置一个单独的变量，用于记录遍历的次序

![](../img/assets_2024-01-19-97算法/2024-02-02-19-24-56-image.png)
...
数据传输的替代性，是指通过不同顺序求解同一目标，要考虑什么方式最简洁
...
## A1｜L1 栈（Stack）与双序数据传输

栈如何支持先序和后续数据传输？



![](../img/assets_2024-01-19-97算法/2024-02-02-19-42-35-image.png)

#### 先序数据传输

首先让A入栈，由于存在 A 到 B 这条边，A1 可以将值传给 B1，B入栈，我们便完成了从先序的数据传输。

#### 后序数据传输

然后 B 出栈，由于A 此时在栈顶，B才可以访问到 A（这一条边是不存在的），故而 B2 将值传递给 A2，我们便完成了后序的数据传输。

综上所述，栈同时**支持两种先序和后序两种顺序的的数据传输**。

> 也就是说，入栈的过程是先序数据传输，出栈的过程是后续数据传输



## A3｜L1 递归的双序数据传输

![](../img/assets_2024-01-19-97算法/2024-02-02-20-24-21-image.png)

先序数据传输可以通过**参数列表**实现，后序数据传输可以通过**返回值**实现。

> 如果是先序，也就是从DFS(1)到DFS(2)的过程，也就是从A到B的过程，值的传递就要通过参数`dfs(x, val)`之中的 val 实现。 如果从B到A，也就是从DFS(1)到 DFS(2) 的过程，可以用返回值来实现，也就是用什么东西接住整个 `dsf(x, val)`的返回值。

这张图有点难以理解，做点详细解释：

我们用 DFS(1) 来**代表 A 的状态**，用 DFS(2) 来**代表 B 的状态**。

**注意：f(x)=2x 这条边，可以在DFS(1) -> DFS(2)，也就是A->B中实现，实现时候要对val做操作，也可以在 DFS(2)->DFS(1), 也就是 B->A 之中实现，实现的时候要通过返回值。但是不能重复实现！**
...
```java
    static int dfs(int x, int val) {
        if (x >= 2) {
//            x >=2 说明在dfs(2), 本期之中改成 x==2 也可以，只有两个状态
//            选择在b之中进行dfs(1) -> dfs(2) 的 f(x) = 2x, 接收A1，其是前序，所以通过参数传递
            b1 = val * 2;
...
课后题：请根据课程代码简单改编，完成A​​➡️​​B​​➡️​​A 的代码块，A的初始值为 1，边的函数关系为f(x)=2x，即 B的值为 2，然后 B 将 2 又返还给 A，A 的值为 4。
